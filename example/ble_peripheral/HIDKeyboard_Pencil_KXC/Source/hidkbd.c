...
static	uint8	advData[] =
{
    ...
    LO_UINT16(GAP_APPEARE_HID_DIGITAL_PEN),
    HI_UINT16(GAP_APPEARE_HID_DIGITAL_PEN),
...
static	uint8	attDeviceName[GAP_DEVICE_NAME_LEN] = "Pencil";
...
//	pGlobal_config[SLAVE_CONN_DELAY_BEFORE_SYNC] = 1100;  //800-1100
	...
	// Setup the GAP Peripheral Role Profile
	{
		...
		uint8	LC_Dev_Mac[7]	=	{0, 0, 0, 0, 0, 0, 0};
		// osal_snv_read(SNV_DEV_MAC_ADDR, 7, LC_Dev_Mac);
		// if(LC_Dev_Mac[0] == 0xaa)
		// {
		// 	VOID osal_memcpy(scanData + SCANF_MAC_OFFSET, LC_Dev_Mac + 1, 6);
		// 	for(uint8 i= 0;i<6;i++)                                                                                                                                                                                                                                             
		// 	{
		// 		ownPublicAddr[i] =	LC_Dev_Mac[6 - i];
		// 	}
		// 	HCI_ReadBDADDRCmd();
		// }
		// else
		{
			hal_flash_read(0x4004,LC_Dev_Mac+1,2);
			hal_flash_read(0x4000,LC_Dev_Mac+3,4);
			VOID osal_memcpy(scanData + SCANF_MAC_OFFSET, LC_Dev_Mac + 1, 6);
		}
		LOG("MAC:");
		LOG_DUMP_BYTE(LC_Dev_Mac + 1, 6);
	...
	appearence_data[0]	=	LO_UINT16(GAP_APPEARE_HID_DIGITAL_PEN);//GAP_APPEARE_HID_KEYBOARD
	appearence_data[1]	=	HI_UINT16(GAP_APPEARE_HID_DIGITAL_PEN);//GAP_APPEARE_HID_KEYBOARD
	...
	// DO NOT set MTUSize,iphone will set the size so that we can identify phone type.
 ...
 * @return  none
 */
void hidKbdSendReport(uint8 combine_enabel, uint8 keycode )
{
	uint8 buf[HID_KEYBOARD_IN_RPT_LEN];
	if(combine_enabel == 1)
	{
		if(LC_Dev_System_Param.dev_phone_type == OLD_PHONE)
		{
			buf[0]	=	0x01;
		}
		else if(LC_Dev_System_Param.dev_phone_type == NEW_PHONE)
		{
			buf[0]	=	0x08;
		}
	}
