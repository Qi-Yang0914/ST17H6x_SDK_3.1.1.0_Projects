...
//	LOG("%s\n",__FUNCTION__);
	...
	if( events & HID_UPPARAM_EVT)
	{
		...
		else if(mtusize == 158 || mtusize == 77)
		{
			LC_Dev_System_Param.dev_phone_type	=	NEW_PHONE;
		}
		else
		{
			if(LC_Dev_System_Param.dev_phone_type == NEW_PHONE)
			{
				LC_Dev_System_Param.dev_phone_type	=	NEW_PHONE;
			}
			else
	...
	else if ( uuid == REPORT_MAP_UUID )
	{
		...
		else
		{
			...
			*pLen = MIN( maxLen, (hidReportMapLen - offset) );
            LOG("HID_MAP_LEN=%d,max=%d,*pLen=%d,mtu=%d,offset=%d\n\r",hidReportMapLen, maxLen, *pLen, ATT_GetCurrentMTUSize(connHandle),offset);
			// copy data
