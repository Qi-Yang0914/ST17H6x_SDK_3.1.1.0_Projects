...
#define	BATT_POWER_STATE_IDX		6 // Position of battery power state in attribute array
#define	BATT_POWER_STATE_CCCD_IDX	7 // Position of battery power state CCCD in attribute array
...
CONST uint8	battPowerStateUUID[ATT_BT_UUID_SIZE]	=	
{
...
// Battery power state Characteristic
static	uint8	battPowerStateProps	=	GATT_PROP_READ | GATT_PROP_NOTIFY;
uint8	battPowerState	=	0xAF;
...
static gattAttribute_t battAttrTbl[] =
{
    ...
    // Battery Power State Client Characteristic Configuration
    {
        ...
        GATT_PERMIT_READ | GATT_PERMIT_WRITE,
        0,
        (uint8*)& battPowerStateClientCharCfg
    },
};
                                  ...
                                  uint8* pValue, uint16 len, uint16 offset )
{
    ...
    case GATT_CLIENT_CHAR_CFG_UUID:
		if(pAttr->handle == battAttrTbl[BATT_LEVEL_VALUE_CCCD_IDX].handle)
		{
		...
		else if(pAttr->handle == battAttrTbl[BATT_POWER_STATE_CCCD_IDX].handle)
		{
...
static void battNotifyCB( linkDBItem_t* pLinkItem )
{
    if ( pLinkItem->stateFlags & LINK_CONNECTED )
    {
		...
		value	=	GATTServApp_ReadCharCfg(pLinkItem->connectionHandle, battPowerStateClientCharCfg);
		if(value & GATT_CLIENT_CFG_NOTIFY)
		{
			attHandleValueNoti_t	noti;
			noti.handle	=	battAttrTbl[BATT_POWER_STATE_IDX].handle;
			noti.len	=	1;
			noti.value[0]	=	battPowerState;
			GATT_Notification(pLinkItem->connectionHandle, &noti, FALSE);
		}
    }
}
